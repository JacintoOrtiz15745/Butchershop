import {View, Text, StatusBar, TouchableOpacity, TextInput, KeyboardAvoidingView, Image, Alert} from 'react-native';
...
const Login = ({navigation}) => { 
    ...
    AsyncStorage.getItem('token').then((res) => {
        navigation.navigate( res ? 'Home' : 'Login') 
    });
  ...
  return ( 
    <KeyboardAvoidingView style={styles.mainContainer}>
        ...
        <TextInput value={inputs.email} onChangeText={suscribe('email')} style={styles.inputMail} placeholder='Email'/>

        <Text style={styles.textPassword}>{text.PASSWORD}</Text>
        <View style={styles.mainContainerPassword}> 
            <TextInput value={inputs.password} onChangeText={suscribe('password')} style={styles.inputPassword} placeholder='Password' secureTextEntry={true}/>
        ...
        <TouchableOpacity style={styles.button} onPress={handleSubmit}><Text style={styles.buttonText}>{text.LOGIN}</Text></TouchableOpacity>
